...
|  |  | 何かが起こる前に特定のコードが実行されることを確認する必要がある場合に便利です。<br>If the PreFlow is in a target endpoint, it executes after the proxy endpoint's PostFlow.<br> PreFlow がターゲットエンドポイントにある場合、プロキシエンドポイントの PostFlow の後に実行されます。 |
